...
export default function () {
  ...
  const loginSuccess = check(loginRes, {
    ...
    'login duration <1s': (r) => r.timings.duration < 1000, // проверка времени логина
  });

  if (!loginSuccess) {
    console.log(`❌ Login failed or too slow for ${account}: ${loginRes.status}, duration=${loginRes.timings.duration}ms`);
    return;
  }
  ...
  const gameSuccess = check(gameRes, {
    ...
    'game duration <1s': (r) => r.timings.duration < 1000, // проверка времени запроса игры
  });

  console.log(`✅ VU=${__VU} account=${account} clicked game, status=${gameRes.status}, duration=${gameRes.timings.duration}ms, success=${gameSuccess}`);
}
