...
import { Counter } from 'k6/metrics';  // <-- правильно
...
export const options = {
  ...
  thresholds: {
    http_status_0: ['count==0'],       // не допускаем обрывов
    http_req_failed: ['rate<0.20'],    // диагностич.
    http_req_duration: ['p(95)<60000'],  // диагностич.
  },
...
export default function () {
  ...
  // увеличенный таймаут, чтобы не ловить status=0
  const res = http.post(URL, JSON.stringify(payload), { headers: HEADERS, timeout: '70s' });
  const ms = Math.round(res.timings.duration);

  if (res.status === 0) {
    status0.add(1);
    console.log(`[ERR 0] email=${EMAIL} ${ms}ms :: interrupted/timeout`);
    return;
  }
