...
test('@Regress back button', async ({ page }) => {
...
await page.goto('https://luckystake.com/game/real/44287');
...
screenshot = await page.screenshot({ fullPage: true });
    test.info().attach(`game is loaded`, {
      body: screenshot,
      contentType: 'image/png', 
    });
await delay5Seconds(); 
...
screenshot = await page.screenshot({ fullPage: true });
    test.info().attach(`back to game lobby`, {
      body: screenshot,
      contentType: 'image/png', 
    });
await delay5Seconds();
