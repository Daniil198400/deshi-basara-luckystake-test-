...
// Массив ID игр
const gameIds = [
  28546, 28565, 28603, 28596, 40489, 40490, 38799, 28591, 35543,
  ...
  28544, 28599, 28537, 28578, 28573, 28588, 28612, 28607, 28597, 28549, 
...
async function clickGameElement(page: Page) {
  ...
  for (const c of candidates) {
    try {
      if (c.type === 'role') {
        const el = gameFrame.getByRole(c.role, { name: c.name, exact: c.exact ?? false });
        if (await el.count()) {
