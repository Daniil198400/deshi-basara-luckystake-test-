...
const SCALE        = 1;      // мягкий масштаб
const AVG_IT_SEC   = 2.0;    // средняя длительность итерации
const AFFILIATE_ID = '';
const RETRY_429    = true;

/* Умеренный стресс: 30s разогрев @1*r, 20s «полушип» @1.5*r, 40s восстановление @1*r */
const WARM_RPS   = Math.max(1, Math.round(1.0 * SCALE));
const SPIKE_RPS  = Math.max(1, Math.round(1.5 * WARM_RPS));  // было 3*r → стало 1.5*r
const REC_RPS    = Math.max(1, Math.round(1.0 * SCALE));
...
export const options = {
  ...
  scenarios: {
    warm:    { executor: 'constant-arrival-rate', rate: WARM_RPS,  timeUnit:'1s', duration:'30s', preAllocatedVUs: PRE_VUS, maxVUs: MAX_VUS, tags:{step:'warm'} },
    spike:   { executor: 'constant-arrival-rate', rate: SPIKE_RPS, timeUnit:'1s', duration:'20s', preAllocatedVUs: PRE_VUS, maxVUs: MAX_VUS, tags:{step:'spike'}, startTime:'30s' },
    recover: { executor: 'constant-arrival-rate', rate: REC_RPS,   timeUnit:'1s', duration:'40s', preAllocatedVUs: PRE_VUS, maxVUs: MAX_VUS, tags:{step:'recover'}, startTime:'50s' },
  },
  thresholds: {
    'signup_success_rate{step:warm}':    ['rate>0.60'], // было 0.80
    'signup_success_rate{step:spike}':   ['rate>0.10'], // было 0.30
    'signup_success_rate{step:recover}': ['rate>0.60'], // было 0.80
...
function makePayload(){
  ...
  const affiliateInformation = {
    appsFlyerId: uuid4(),
    mediaSource: 'k6-load',
    campaign: 'signup-stress-moderate',
    adset: 'k6',
    ad: 'k6',
    clickId: uuid4(),
  };
...
export default function () {
  // чуть больше джиттера, чтобы рассинхронизировать запросы
  ...
  // один ретрай 429 с коротким backoff
  if (RETRY_429 && res.status === 429) {
    const ra = Number(res.headers['Retry-After'] || 0);
    sleep(Math.min(Math.max(ra, 0.3), 0.8));
  ...
  if (ok) {
    ...
    console.warn(`[${step}] FAIL ${res.status} email=${payload.email} phone=${payload.phoneNumber}` +
