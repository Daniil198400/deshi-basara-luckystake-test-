...
// ——— утилиты ———
async function visible(locator: Locator, timeout = 5000) {
  try {
...
test('Four7', async ({ page }, testInfo) => {
  ...
  await page.goto('https://luckystake.dev/game/real/29136');
  await delay10Seconds();

  // скрин до Play now
  let screenshot = await page.screenshot({ fullPage: true });
  await testInfo.attach('game_before_playNow', { body: screenshot, contentType: 'image/png' });
  await delay5Seconds();
  ...
  await testInfo.attach('game_after_wait', { body: screenshot, contentType: 'image/png' });

  // Explore games → если кликнули, пропускаем поиск
  ...
  if (!skipSearch) {
    ...
    if (await visible(searchButton.first(), 3000)) {
      ...
      await testInfo.attach('game_after_search_button', { body: shot, contentType: 'image/png' });
  ...
  if (await clickIfVisible(buyButton, 10000)) {
    ...
    await testInfo.attach('game_buy_button', { body: shot, contentType: 'image/png' });
    ...
    if (await clickIfVisible(priceButton, 10000)) {
      ...
      await testInfo.attach('game_card_proposition_after_clicking_on_random_price', {
        body: afterPrice,
        contentType: 'image/png',
      });

      // confirm (nth(2) лучше заменить на стабильный локатор, если знаешь)
